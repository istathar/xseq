...
/**
 * This unit test requires that that there is a jabber server at localhost, that
 * it speak SSL (we'll verify that) and that it support account creation. A test
 * user will be created and destroyed.
 * 
 * <P>
 ...
 * org.jivesoftware.smack means the imports below are explicit and obvious, and
 * more importantly, helps us make sure we're not using package scope classes or
 * methods.
 * 
 * @author Andrew Cowie
 */
public class BasicSmackTest extends TestCase
{
	...
	public void testDeleteAccount() {
		try {
			...
			try {
				...
				XMPPError err = xe.getXMPPError();
				System.err.println("Received error code: " + err.getCode() + ", message: " + err.getMessage());
	...
	/**
	 * This little silly "test" is just to simulate a Press Any Key to Continue
	 * in the console if you're using the Smack debugger as it disappears with
	 * app termination (rather than holding the app running until its windows is
	 * closed)
	 *  
	 */
