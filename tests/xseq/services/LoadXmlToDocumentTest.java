...
public class LoadXmlToDocumentTest extends TestCase
{
	...
	/**
	 * Test the fileToString utility method. This is a bit overdone, but it's a
	 * useful test to make sure files are in place, etc. Also, note that it will
	 * blow up if done a platform that doesn't have \n as newline. Ah, Java code
	 * that won't work on Windows. Gotta love it.
	 */
	...
	public void testLoadXmlToDocument() {
		...
		// On the assumption that we will be receiving our XML in string blobs,
		...
		assertEquals("document type declaration isn't procedure (you are using the right test data, right?) ",
		...
		assertEquals("first node of NodeList isn't an element ", procedure.getNodeType(), Node.ELEMENT_NODE);
		assertEquals("first element isn't <procedure> as expected ", procedure.getNodeName(), "procedure");

		Element title = (Element) nodelist.item(1);
		assertEquals("next element of NodeList isn't an element ", title.getNodeType(), Node.ELEMENT_NODE);
