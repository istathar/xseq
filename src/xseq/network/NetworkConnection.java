...
/**
 * Holds or points to all the pertinent information regarding our connection to
 * the Jabber network and to whom the master client is. A singleton is held by
 * ProcedureClient, which provides an entry point to all the communication APIs
 * in the xseq.network services layer.
 * 
 * @author Andrew Cowie
 */
public class NetworkConnection
{
	...
	/**
	 * Set the Jabber connection parameters. Not that the act of setting any new
	 * values on parameters causes the validated flag to be unset.
	 * 
	 * @param username
	 * @param server
	 * @param password
	...
	public void login() throws IllegalArgumentException {
		if (_conn == null) {
			throw new DebugException("How did you manage to try to login() without having first connect()ed?");
		}
		try {
