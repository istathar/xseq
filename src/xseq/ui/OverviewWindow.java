...
public class OverviewWindow
{
	 ...
	 * not? - This should be a member of the (a?) MasterWindow if more than one
	 * OverviewWindow can be open? If so, then we'll need to add get/setModels,
	 * and move the create code below elsewhere.]
	 */

	// the instantiated TreeModel (as impleneted by
	// ListStore). Doesn't need to be static, for if we
	// ever do instantiate another OverviewWindow, we can just give it the same
	...
	public OverviewWindow(Procedure p) {
		...
		try {
			_glade = new LibGlade("share/overview.glade", this);
		...
		TreeViewColumn viewColumn0 = new TreeViewColumn();
		...
		CellRendererText renderer0 = new CellRendererText();
		viewColumn0.packStart(renderer0, true);
		viewColumn0.addAttributeMapping(renderer0, CellRendererText.Attribute.MARKUP, _sectionDataColumns[0]);
		_view.appendColumn(viewColumn0);

		_view.setAlternateRowColor(true);
		...
		_view.addListener(new TreeViewListener() {
			public void treeViewEvent(TreeViewEvent event) {
				// so silly. You'd think that TreePath would have an int return
				// for ListStores. <sigh>
	...
	/**
	 * Given a <section>Element, generate the Label which summarizes the details
	 * of the <steps>in that <section>. There is Pango Markup here!!!
	 * 
	 * HARDCODE This is really tightly tied to the procedure DTD, getting titles
	 * from attributes, etc.
	 */
	private static String sectionToPango(Element section) {
		...
		if (precisList.getLength() == 1) {
			...
			text.append("<i>");
			// TODO this is probably not sufficient if there are any child nodes
			 ...
			 * expressing auto word wrap, and worse, GtkCellRendererText has no
	...
	//	System.out.println("debug: treeviewactivated, " + cursorPath.toString());
	//}
	public void activateSection(int index, boolean containsCurrentStep) {
		if ((index < 0) && (index >= _numSections)) {
			throw new DebugException("OverviewWindow's activateSection() was called with an illegal section number, "
					+ index);
		}
		 ...
		 * the "Current" button hot; otherwise we do.
		 */
		...
		/*
		 * Fairly straight forward - just scroll the TreeView to show the active
		 * section.
		 */
		TreePath tp = new TreePath(Integer.toString(index));
		// bizarely, this does strange things. Although it does seem to set the
