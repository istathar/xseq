...
public class Procedure
{
	...
	public Procedure(String xml) {
		...
		/*
		 * steps number consecutively across the entire procedure document; they
		 * don't restart across section boundaries.
		 */
		...
		for (int k = 0; k < num_steps; k++) {
			...
			for (int m = 0; m < num_names; m++) {
				...
				/*
				 * We skip setting a num attribute on name as it doesn't have a
				 * canonical number
				 */
	 ...
	 * 
	 * @param taskId
	 * @return the next task [id] in this step, or null if all the tasks in this
	 *         step for this person are done.
	 */
	 ...
	 * 
	 * @param thisTaskId
	 * @return null if all the tasks for [this person in] this step are complete
	 *         (as judged by sequential ordering; preceding tasks before
	 *         thisTaskId are assumed to already be complete).
	 */
	...
	/*
	 * Actually do the work. This is admittedly tricky code as you have to fight
	 * through the non-Element Nodes to get to the next Element sibbling.
	 */
	private String getNextTask(Element thisTask) {
		Node sibling = (Node) thisTask;
	...
	/**
	 * Utility method to get the id of a given parent working from a given child
	 * node (ie, a task). This is needed for things like looking up TextTags to
	 * adjust the presentation in DetailsWindow.
	 * 
	 * @param id
	...
	/**
	 * Decend down from a node as specified by ID, and find the first Task's ID.
	 * This is used as we cross from one step or section to the next, to pull
	 * out the next task to be lit up.
	 * 
	 * @param parentId
	 ...
	 * @return the ID string of the next step, or null if we don't know of one.
	 */
	...
	/**
	 * Quick method to determine whether a task (specified by ID), belongs to a
	 * particular person.
	 * 
	 * @param taskId
